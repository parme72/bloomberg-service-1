...
(async function () {
    ...
    app.use('/bloomberg-service/demo/app.json', (req, res) => {
        ...
        appConfig.startup_app.url = url.resolve(siteRoot, appConfig.startup_app.url);
        
        if (!isProduction) {
            appConfig.runtime.version = runtimeVersion;            
            appConfig.services[0].manifestUrl = url.resolve(siteRoot, '/bloomberg-service/provider/app.json');
        }
    ...
    app.use('/bloomberg-service/provider/app.json', (req, res) => {
        ...
        if (!isProduction) {
            ...
            appConfig.startup_app.url = url.resolve(siteRoot, '/bloomberg-service/provider/index.html');
            appConfig.appAssets[0].src = url.resolve(siteRoot, '/bloomberg-service/provider/assets/OpenFin.BloombergService.zip');
